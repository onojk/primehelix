...
\title{On the Distribution of Smaller Factors in Semiprimes:\\
  Discrete Activation Thresholds, Slow Convergence, and a Refined Heuristic}
...
by a single subleading term, we fit six single-parameter models to the residuals
\[
  r(N,\theta_0) = \delta(N,\theta_0)\cdot\log\log N - f(\theta_0)
\]
at \(N \in \{10^8,\, 2\times10^8,\, 5\times10^8,\, 10^9\}\) and
\(\theta_0 \in [0.10,0.45]\).  Models tested: \(A\theta_0\), \(A\theta_0^2\),
\(A\theta_0\log(1/\theta_0)\), \(A(\log 2 - \theta_0)\),
\(A/\log\log N\), and \(A\cdot f(\theta_0)\).
No model achieves \(R^2 > 0.97\) (best \(R^2 = 0.25\) for the \(A\theta_0\) form).
The subleading correction structure remains open.
...
\begin{table}[ht]
\centering
\caption{Summary of embeddings tested and observed outcomes.}
...
\subsection{Stronger separation principle}
\label{sec:separation}

\begin{conjecture}[Separation principle]
Let \(\phi:\mathbb{N}\to\mathbb{R}\) be any function that depends only on the multiset
of prime factors of \(n\) in a symmetric way.  Then \(\phi(n)\) does not determine
\(\theta = \log p/\log n\) for a positive-density subsequence of semiprimes.
\end{conjecture}

A proof sketch: for a given \(n = pq\) with \(p < q\), there is no second
factorisation, but \(\phi(n) = \phi(q \cdot p)\) gives the same value regardless of
ordering.  Any \(\phi\) that is insensitive to which of \(p, q\) is the smallest
factor can at best determine \(\theta\) up to the reflection \(\theta \leftrightarrow 1-\theta\).
For \(\theta < 1/2\) this reflection gives a different value; the ambiguity is inherent
to any function that does not explicitly use \(\mathrm{spf}(n)\) or an equivalent.
