...
\footnote{The PrimeHelix tool (\url{https://github.com/onojk/primehelix})
uses an alternative lopsidedness criterion based on the geometric
balance condition $(q/p - 1)/\sqrt{q/p} = 10$, giving a ratio
threshold $r = q/p \geq 101.99$.  This is a ratio-based (absolute)
criterion, in contrast to the projective criterion $\theta \leq
\theta_0$ used in this paper.  The two notions coincide only
approximately, with the ratio boundary corresponding to
$\theta \approx 0.33$--$0.38$ depending on $\log n$.  The
projective coordinate $\theta$ is preferred here because it is
scale-invariant and admits a clean asymptotic analysis.},
...
\caption{Factor-ratio \(\log(p/q)\) versus \(\theta\) for all semiprimes
  \(n \leq 10^6\) (30\,000 sampled points; colour encodes \(\log n\)).
  Thin grey lines show the exact identity \(\log(p/q)=(2\theta-1)\log n\)
  at \(\log n \in \{8,10,12,14,16,18\}\); the dashed red line is at
  the median \(\log n = 13.09\).  The scatter lies exactly on this
  family, confirming identity~\eqref{eq:identity}.}
\label{fig:scatter}
\end{figure}
...
The data and heuristic are consistent with
\(\lim_{N\to\infty}P(N,\theta_0)=1\) for fixed
\(\theta_0\in(0,\tfrac12)\), though this limit remains unproved.  Thus the random variable \(\theta_N\) (for a
...
very small (Montgomery--Vaughan~\cite{montgomery2007}).  This concentration
...
the asymptotic rate is known to involve \(O(1/\!\log\log N)\) decays,
...
We note that \(\alpha \approx 3.056\) is close to the integer \(3\); whether
\(b(\theta_0) \propto (1-\theta_0)^3\) has a theoretical basis remains an open question.
...
can capture.  The residual structure reflects a superposition of prime-indexed
activation events and is therefore not expected to admit a
low-dimensional parametric model; the sign change near
\(\theta_0 \approx 0.20\) is consistent with the transition between
small-\(p\) and large-\(p\) dominated activation regimes.
...
(Montgomery--Vaughan~\cite{montgomery2007}).  The local linear-in-\(\log N\)
...
\begin{itemize}
  ...
  \item Test whether \(P(\theta \le \theta_0) \approx 1 - \rho(1/\theta_0)\)
        holds approximately, where \(\rho\) is the Dickman function; if so,
        this would connect the deficit directly to the classical smooth-number
        literature.
\end{itemize}
